        ...
        <div class="table-wrapper">
            <div class="table-title">
                <div class="row">
                    ...
                    <div class="col-sm-6">
                        <a href="#addEmployeeModal" class="btn btn-success" data-toggle="modal"><i class="material-icons">&#xE147;</i> <span>Agregar Nuevo Tipo</span></a>
            ...
            <div class="modal-content">
                <form>
                    ...
                    <div class="modal-body">
                        <div class="form-group">
                            <label>ID</label>
                            <input id="id_producto" type="text" class="form-control" disabled>
                        </div>
                        <div class="form-group">
                            <label>Nombre</label>
                        ...
                        <div class="form-group">
                            <label>Precio</label>
                            <input id="precio_edit" type="text" class="form-control" required>
                        </div>
                        <div class="form-group">
                            <label>Cantidad</label>
                            <input id="cantidad_edit" type="text" class="form-control" required>
                        </div>
                        <div class="form-group">
                            <label>Tipo</label>
                            <select id="tipo_edit" type="text" class="form-control" required>
                                <!-- Opciones de raza se poblarán dinámicamente -->
                            </select>
                        </div>
                        <div class="form-group">
        ...
        $(document).ready(function() {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    response.productos.forEach(function(usuario) {
                        var row = '<tr>' +
                            '<td>' +
                            // '<span class="custom-checkbox">' +
                            ...
                            '<td>' + usuario.cantidad_disponible + '</td>' +
                            '<td>' + usuario.tipo_producto_nombre + '</td>' +
                ...
                error: function(xhr, status, error) {
                    console.error('Error al obtener los datos de los tipos de Productos:', error);
                }
            });
            ...
            $('table').on('click', '.edit', function() {
                ...
                var id_producto = currentRow.find('td:eq(1)').text();
                var nombre = currentRow.find('td:eq(2)').text();
                var descripcion = currentRow.find('td:eq(3)').text();
                var precio = currentRow.find('td:eq(4)').text();
                var cantidad = currentRow.find('td:eq(5)').text();
                var tipo_producto = currentRow.find('td:eq(6)').text();
                var estado = currentRow.find('td:eq(7)').text();
                ...
                // $('#editEmployeeModal input[id="tipo_edit"]').val(tipo);

                // Llenar los campos de la modal de edición con los datos del usuario
                $('#editEmployeeModal input[id="id_producto"]').val(id_producto);
                $('#editEmployeeModal input[id="nombre_edit"]').val(nombre);
                $('#editEmployeeModal input[id="descripcion_edit"]').val(descripcion);
                $('#editEmployeeModal input[id="precio_edit"]').val(precio);
                $('#editEmployeeModal input[id="cantidad_edit"]').val(cantidad);
                $('#editEmployeeModal input[id="tipo_edit"]').val(tipo_producto);
            ...
            document.getElementById("editEmployeeModal").addEventListener("submit", function(event) {
                ...
                var descripcion = document.getElementById("descripcion_edit").value;
                var precio = document.getElementById("precio_edit").value.trim();
                var cantidad = document.getElementById("cantidad_edit").value.trim();
                var tipo_producto = document.getElementById("tipo_edit").value.trim();
                var estado = document.getElementById("estado_edit").value.trim();

                // Validar que los campos no estén vacíos
                if (id_producto === "" || nombre === "" || descripcion === "" || precio === "" || cantidad === "" || tipo_producto === "" || estado === "") {
                ...
                formData.append('cantidad_disponible', cantidad);
                formData.append('id_tipo_producto', tipo_producto);
                ...
                xhr.open("POST", "<?php echo $server_url; ?>/productos.php");
                xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded;charset=UTF-8");
                xhr.onload = function() {
                    ...
                    if (xhr.status === 200) {
                        ...
                        if (xhr.responseText == "Producto actualizado exitosamente") {
            ...
            document.getElementById("addEmployeeModal").addEventListener("submit", function(event) {
                ...
                var tipo_producto = document.getElementById("tipo").value.trim();
                var estado = document.getElementById("estado").value.trim();

                // Validar que los campos no estén vacíos
                if (nombre === "" || descripcion === "" || precio === "" || cantidad === "" || tipo_producto === "" || estado === "") {
                    alert("Por favor, complete todos los campos.");
                    return;
                }
                ...
                xhr.onload = function() {
                    if (xhr.status === 200) {
                        // Si la respuesta es exitosa, consultar el contenido de la respuesta
                        // var response = JSON.parse(xhr.responseText);
                        if (xhr.responseText == "Producto creado exitosamente") {
                            // Si la respuesta indica éxito, mostrar el modal de éxito con el mensaje proporcionado por el servidor
                            var successMessage = xhr.responseText;
                            document.getElementById("successModalLabel").innerText = "Se realizo con Éxito";
                            document.getElementById("successMessage").innerText = successMessage;
            ...
            $('table').on('click', '.delete', function() {
                ...
                document.getElementById("deleteEmployeeModal").addEventListener("submit", function(event) {
                    ...
                    var formData = new URLSearchParams();
                    formData.append('id_producto', id_usuario);
                    ...
                    xhr.onload = function() {
                        ...
                        if (xhr.status === 200) {
                            // Si la respuesta es exitosa, consultar el contenido de la respuesta
                            // var response = JSON.parse(xhr.responseText);
                            if (xhr.responseText == "Producto eliminado exitosamente") {
                                // Si la respuesta indica éxito, mostrar el modal de éxito con el mensaje proporcionado por el servidor
                                var successMessage = xhr.responseText;
                                document.getElementById("successModalLabel").innerText = "Se realizo con Éxito";
                                document.getElementById("successMessage").innerText = successMessage;
            ...
            $.ajax({
                url: '<?php echo $server_url; ?>/tipos_productos.php', // Endpoint para obtener datos de usuarios y razas
                type: 'POST',
                data: {
                    accion: 'listar',
                    estado: 'activo'
                ...
                success: function(response) {
                    ...
                    response.tiposProductos.forEach(function(usuario) {
                        // Por ejemplo, puedes agregar opciones a un campo de selección de usuarios
                        $('#tipo').append($('<option>', {
                            value: usuario.id,
                            text: usuario.nombre_tipo
                        }));
                        $('#tipo_edit').append($('<option>', {
                            value: usuario.id,
                            text: usuario.nombre_tipo
                        }));
                    });
