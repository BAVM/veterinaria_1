...
<body>
    <div class="container">
        <div class="table-wrapper">
            <div class="table-title">
                <div class="row">
                    <div class="col-sm-6">
                        <h2>Gestión de <b>Mascotas</b></h2>
                    </div>
                    <div class="col-sm-6">
                        <a href="#addEmployeeModal" class="btn btn-success" data-toggle="modal"><i class="material-icons">&#xE147;</i> <span>Agregar Nueva Mascota</span></a>
            ...
            <div class="modal-content">
                <form>
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-group">
                            <label>Raza</label>
                            <select id="raza" type="text" class="form-control" required>
            ...
            <div class="modal-content">
                <form>
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-group">
                            <label>Fecha de Nacimiento</label>
                            <input id="fechaNacimiento_edit" type="date" class="form-control" required>
                        </div>
                        <div class="form-group">
                            <label>Raza</label>
                            <select id="raza_edit" type="text" class="form-control" required>
                            </select>
                        </div>
                        <div class="form-group">
                            <label>Dueño</label>
    ...
    <div id="deleteEmployeeModal" class="modal fade">
        <div class="modal-dialog">
            <div class="modal-content">
                <form>
                    <div class="modal-header">
                        <h4 class="modal-title">Borrar Mascota</h4>
                        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                    </div>
                    <div class="modal-body">
    ...
    <script>
        $(document).ready(function() {
            ...
            $.ajax({
                url: '<?php echo $server_url; ?>/mascotas.php',
                ...
                success: function(response) {
                    ...
                    response.mascotas.forEach(function(usuario) {
                        var row = '<tr>' +
                            '<td>' +
                            // '<span class="custom-checkbox">' +
                            ...
                            '<td>' + usuario.nombre_raza + '</td>' +
                            '<td>' + usuario.nombre_animal + '</td>' +
                            '<td>' + usuario.dueño + '</td>' +
            ...
            $('table').on('click', '.edit', function() {
                ...
                var fecha_nacimiento = currentRow.find('td:eq(3)').text();
                var raza = currentRow.find('td:eq(4)').text();
                var dueño = currentRow.find('td:eq(6)').text();
                ...
                $('#editEmployeeModal input[id="fechaNacimiento_edit"]').val(fecha_nacimiento);
                $('#editEmployeeModal input[id="raza_edit"]').val(raza);
                $('#editEmployeeModal input[id="cliente_edit"]').val(dueño);
                $('#editEmployeeModal input[id="estado_edit"]').val(estado);

                // Iterar sobre las opciones del campo de selección de dueño
                ...
                $('#raza_edit option').each(function() {
            ...
            document.getElementById("editEmployeeModal").addEventListener("submit", function(event) {
                ...
                var fecha_nacimiento = document.getElementById("fechaNacimiento_edit").value.trim();
                var cliente = document.getElementById("cliente_edit").value.trim();
                var raza = document.getElementById("raza_edit").value.trim();
                var status = document.getElementById("estado_edit").value.trim();


                // Validar que los campos no estén vacíos
                if (firstName === "" || fecha_nacimiento === "" || raza === "" || cliente === "" || status === "") {
                    alert("Por favor, complete todos los campos.");
                    return;
                }

                var formData = new URLSearchParams();
                formData.append('id_mascota', id_user);
                formData.append('nombre_mascota', firstName);
                ...
                xhr.open("POST", "<?php echo $server_url; ?>/mascotas.php");
                xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded;charset=UTF-8");
                xhr.onload = function() {
                    if (xhr.status === 200) {
                        // Si la respuesta es exitosa, consultar el contenido de la respuesta
                        // var response = JSON.parse(xhr.responseText);
                        if (xhr.responseText == "Mascota actualizada exitosamente") {
            ...
            document.getElementById("addEmployeeModal").addEventListener("submit", function(event) {
                ...
                var fecha_nacimiento = document.getElementById("fechaNacimiento").value.trim();
                var raza = document.getElementById("raza").value.trim();
                var cliente = document.getElementById("cliente").value.trim();

                // Validar que los campos no estén vacíos
                if (firstName === "" || fecha_nacimiento === "" || raza === "" || cliente === "") {
                    alert("Por favor, complete todos los campos.");
                    return;
                }
                ...
                xhr.onload = function() {
                    ...
                    if (xhr.status === 200) {
                        // Si la respuesta es exitosa, consultar el contenido de la respuesta
                        // var response = JSON.parse(xhr.responseText);
                        if (xhr.responseText == "Mascota creada exitosamente") {
                            // Si la respuesta indica éxito, mostrar el modal de éxito con el mensaje proporcionado por el servidor
                            var successMessage = xhr.responseText;
                            document.getElementById("successModalLabel").innerText = "Se realizo con Éxito";
                            document.getElementById("successMessage").innerText = successMessage;
            ...
            $('table').on('click', '.delete', function() {
                ...
                document.getElementById("deleteEmployeeModal").addEventListener("submit", function(event) {
                    ...
                    var formData = new URLSearchParams();
                    formData.append('id_mascota', id_usuario);
                    ...
                    xhr.onload = function() {
                        if (xhr.status === 200) {
                            // Si la respuesta es exitosa, consultar el contenido de la respuesta
                            // var response = JSON.parse(xhr.responseText);
                            if (xhr.responseText == "Mascota eliminada exitosamente") {
                                // Si la respuesta indica éxito, mostrar el modal de éxito con el mensaje proporcionado por el servidor
                                var successMessage = xhr.responseText;
                                document.getElementById("successModalLabel").innerText = "Se realizo con Éxito";
                                document.getElementById("successMessage").innerText = successMessage;
            ...
            $.ajax({
                url: '<?php echo $server_url; ?>/razas.php', // Endpoint para obtener datos de usuarios y razas
                type: 'POST',
                data: {
                    accion: 'listar',
                    estado: 'activo'
                ...
                success: function(response) {
                    // Poblar campos de formulario de edición con datos de usuarios y razas
                    response.razas.forEach(function(usuario) {
                        // Por ejemplo, puedes agregar opciones a un campo de selección de usuarios
                        $('#raza_edit').append($('<option>', {
                            value: usuario.id,
                            text: usuario.nombre_raza
                        }));
                        $('#raza').append($('<option>', {
                            value: usuario.id,
                            text: usuario.nombre_raza
