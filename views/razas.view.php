...
<body>
    <div class="container">
        <div class="table-wrapper">
            <div class="table-title">
                <div class="row">
                    <div class="col-sm-6">
                        <h2>Gestión de <b>Tipos de Razas</b></h2>
                    </div>
                    <div class="col-sm-6">
    ...
    <div id="editEmployeeModal" class="modal fade">
        <div class="modal-dialog">
            <div class="modal-content">
                <form>
                    <div class="modal-header">
                        <h4 class="modal-title">Editar Raza</h4>
                        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                    </div>
                    <div class="modal-body">
                        <div class="form-group">
    ...
    <div id="deleteEmployeeModal" class="modal fade">
        <div class="modal-dialog">
            <div class="modal-content">
                <form>
                    <div class="modal-header">
                        <h4 class="modal-title">Borrar Raza</h4>
                        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                    </div>
                    <div class="modal-body">
    ...
    <script>
        $(document).ready(function() {
            ...
            $.ajax({
                url: '<?php echo $server_url; ?>/razas.php',
            ...
            document.getElementById("editEmployeeModal").addEventListener("submit", function(event) {
                ...
                var tipo = document.getElementById("tipo_edit").value.trim();
                var status = document.getElementById("estado_edit").value.trim();

                // Validar que los campos no estén vacíos
                if (firstName === "" || tipo === "" || status === "") {
                ...
                formData.append('id_raza', id_user);
                formData.append('nombre_raza', firstName);
                formData.append('id_tipo_animal', tipo);
                formData.append('estado', status === '1' ? 'activo' : 'inactivo');
                formData.append('accion', 'editar');

                // Hacer la petición HTTP al servidor
                var xhr = new XMLHttpRequest();
                xhr.open("POST", "<?php echo $server_url; ?>/razas.php");
                xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded;charset=UTF-8");
                xhr.onload = function() {
                    ...
                    if (xhr.status === 200) {
                        ...
                        if (xhr.responseText == "Raza actualizada exitosamente") {
            ...
            document.getElementById("addEmployeeModal").addEventListener("submit", function(event) {
                ...
                xhr.onload = function() {
                    ...
                    if (xhr.status === 200) {
                        // Si la respuesta es exitosa, consultar el contenido de la respuesta
                        // var response = JSON.parse(xhr.responseText);
                        if (xhr.responseText == "Raza creada exitosamente") {
                            // Si la respuesta indica éxito, mostrar el modal de éxito con el mensaje proporcionado por el servidor
                            var successMessage = xhr.responseText;
                            document.getElementById("successModalLabel").innerText = "Se realizo con Éxito";
                            document.getElementById("successMessage").innerText = successMessage;
            ...
            $('table').on('click', '.delete', function() {
                ...
                document.getElementById("deleteEmployeeModal").addEventListener("submit", function(event) {
                    ...
                    var formData = new URLSearchParams();
                    formData.append('id_raza', id_usuario);
                    ...
                    xhr.onload = function() {
                        ...
                        if (xhr.status === 200) {
                            // Si la respuesta es exitosa, consultar el contenido de la respuesta
                            // var response = JSON.parse(xhr.responseText);
                            if (xhr.responseText == "Raza eliminada exitosamente") {
                                // Si la respuesta indica éxito, mostrar el modal de éxito con el mensaje proporcionado por el servidor
                                var successMessage = xhr.responseText;
                                document.getElementById("successModalLabel").innerText = "Se realizo con Éxito";
                                document.getElementById("successMessage").innerText = successMessage;
            ...
            $.ajax({
                url: '<?php echo $server_url; ?>/tipos_animal.php', // Endpoint para obtener datos de usuarios y razas
                type: 'POST',
                data: {
                    accion: 'listar',
                    estado: 'activo'
                ...
                success: function(response) {
                    ...
                    response.tiposAnimales.forEach(function(usuario) {
                        // Por ejemplo, puedes agregar opciones a un campo de selección de usuarios
                        $('#tipo').append($('<option>', {
                            value: usuario.id,
                            text: usuario.nombre_animal
                        }));
